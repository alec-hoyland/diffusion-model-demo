...
def get_pipeline(
    ...
    if precision == 32:
        ...
        raise ValueError(f"Invalid precision {precision}. Needs to be 32 or 16.")
    ...
    if name == "txt2img" and version == "XL 1.0":
        ...
        raise ValueError(f"Invalid pipeline name {name} and version {version}")

    if compile:
        pipe.unet = torch.compile(pipe.unet, mode="reduce_overhead", fullgraph=True)
